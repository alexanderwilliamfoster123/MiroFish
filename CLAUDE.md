...
- **Brand tokens** (from the Paktos brand guidelines): neutral near-black scale; Plum `#6A2E9E` + Lavender `#D3C2F0` accents (`--lbA`, `--lbBsoft`, flipped in dark mode); standout gradient `#F0DCFF → #FF8E94 → #D95DDF → #9081DF` (`--brandGrad`) used sparingly. **No red on losing amounts** — losses render neutral. **No emojis in UI chrome** except country flags, character-face avatars, and icon spots (never inline in text).
